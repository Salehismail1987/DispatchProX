...
declare var $: any;
@Component({
  selector: 'app-freelance-create-ticket',
  templateUrl: './freelance-create-ticket.component.html',
  styleUrls: ['./freelance-create-ticket.component.css']
})

export class FreelanceCreateTicketComponent implements OnInit {
     ...
     if(response.status){
       ...
       swalWithBootstrapButtons.fire(
         ...
         response.message).then(() => {
           ...
           this.reason_decline='';
           window.location.href= '/driver-scheduler?status='+this.status ;
         });
     }else{
        const swalWithBootstrapButtons = Swal.mixin({
         customClass: {
