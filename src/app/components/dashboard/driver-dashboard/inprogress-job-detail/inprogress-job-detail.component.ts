...
@Component({
  selector: 'app-inprogress-job-detail',
  templateUrl: './inprogress-job-detail.component.html',
  styleUrls: ['./inprogress-job-detail.component.css'],

  changeDetection: ChangeDetectionStrategy.OnPush
})


export class InprogressJobDetailComponent implements OnInit {
  ...
  handlePaperTicketId() {
    ...
    let data = {
      ...
      user_id: this.loggedinUser.user_data_request_id ? this.loggedinUser.user_data_request_id : this.loggedinUser?.id,
  ...
  getTicketDetail(shouldUpdateDriverTime = true) {
    ...
    let data = { date: this.date_today, user_id: this.loggedinUser.user_data_request_id ? this.loggedinUser.user_data_request_id : this.loggedinUser?.id, driver_ticket_id: this.driver_ticket_id };
        ...
        if (response.data) {
          if (response.data.all_done) {
            ...
            if ((this.ticket_detail?.is_without_rounds == '1' || this.ticket_detail?.is_without_rounds == 1) && this.ticket_detail?.ticket?.ticket_truck_type_rounds.length == 1) {
              window.location.href = '/inprogress-job-detail/' + this.ticket_detail?.id;
            }
          }
        }
      } else {
  ...
  handleSendApproval() {
    ...
    this.driver_service.sendForApprovel(data).subscribe(response => {
      ...
      if (response.status) {
        ...
        this.router.navigateByUrl('/completed-job-detail/' + this.ticket_detail.id);
      } else {
        const swalWithBootstrapButtons = Swal.mixin({
          customClass: {
            confirmButton: 'btn bg-pink width-200'
  ...
  handleSiteChange() {
    ...
    let data = { round_id: this.update_round_id, dump_site: this.new_dum_site };
  ...
  handleUpdatePickupSite() {
    ...
    let data = { round_id: this.update_round_id, pickup_site: this.pickup_site };
    this.loading_update = true;
  ...
  handleNewMaterial() {
    ...
    let data = { round_id: this.update_round_id, material_taken_out: this.new_material_taken_out };
    this.loading_update = true;
  ...
  handleNewNotes() {
    ...
    let data = { round_id: this.update_round_id, driver_notes: this.driver_round_notes };
    this.loading_update = true;
  ...
  updateJobNotes() {
    ...
    let data = { driver_ticket_id: this.ticket_detail.id, notes_for_approval: this.notes_for_approval };
    this.loading_update = true;
  ...
  handleAddDumpsite() {
    ...
    data.project_id = this.ticket_detail?.ticket?.project.id;
    ...
    this.project_service.addDumpSite(data).subscribe(response => {
      ...
      if (response.status && response.message) {
        ...
        this.show_add_dumpsite = false;
        this.isFormClicked = false;
        this.projectDetails(this.ticket_detail?.ticket?.project.id)
      } else {
  ...
  showEditMaterial(round: any) {

    this.show_round_update_popup = 'material-popup'
    this.update_round_id = round?.id;
    this.new_material_taken_out = round?.material_taken_out;
    setTimeout(() => {
      $("#material_modal").html($("#materialout").html())
    }, 300);
  }
  showEditAddressDS() {
  ...
  handleSetPrefill(round_id:any , ticket_id:any){
    ...
    this.driver_service.updateRoundPrefilled(data).subscribe(response => {
