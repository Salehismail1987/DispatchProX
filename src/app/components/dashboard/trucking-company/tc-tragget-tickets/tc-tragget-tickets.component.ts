...
@Component({
  selector: 'app-tc-tragget-tickets',
  templateUrl: './tc-tragget-tickets.component.html',
  styleUrls: ['./tc-tragget-tickets.component.css'],
})
export class TcTraggetTicketsComponent implements OnInit {
  active_menu: any;
  loggedInUser: any;
    ...
    this.plan_service.getTraggetTickets(formData).subscribe((response) => {
      if (response.status && response.data) {
        ...
        if (this.tragget_tickets?.subscription_upgrade?.quantity) {
          ...
          total_units = this.tragget_tickets?.usage?.total_subs;
        }
  ...
  confirmUpgrade() {
    if (this.upgrade_plan_quantity < 1) {
      Swal.fire({
        ...
        title: `Success!`,
        text: 'For upgrade quantity cannot be 0.',
      }).then(() => { });
      return;
    }
      ...
      .subscribe((response: any) => {
        ...
        if (response && response.status) {
          Swal.fire({
            ...
            this.getTraggetTickets();
            $('#increaseSubscription').modal('hide');
          });
        } else {
          Swal.fire({
            confirmButtonColor: '#17A1FA',
