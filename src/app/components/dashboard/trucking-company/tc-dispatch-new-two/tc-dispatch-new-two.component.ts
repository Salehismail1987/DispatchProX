...
@Component({
  selector: 'app-tc-dispatch-new-two',
  templateUrl: './tc-dispatch-new-two.component.html',
  styleUrls: ['./tc-dispatch-new-two.component.css']
})
export class TcDispatchNewTwoComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    $(document).ready(function () {
      $('.vendor-pay-input-field').focus(function (this: any) {
        $(this).prevAll('.fs-14-400-semi').addClass('vendor-pay-input-field-before');
        $('.vendor-pay-input-box').css("padding", "0px 5px");
      }).blur(function (this: any) {
        $(this).prevAll('.fs-14-400-semi').removeClass('vendor-pay-input-field-before');
        $('.vendor-pay-input-box').css("padding", "1px 5px 4px 5px")
      });
    });
  ...
  showJobApprovers() {
    if ($("#jobApproversPopup").css('display') == 'none') {
      $("#jobApproversPopup").css('display', 'block');
    } else {
      $("#jobApproversPopup").css('display', 'none');
    }
  }
  showTruckType() {
    if ($("#truckTypePopup").css('display') == 'none') {
      $("#truckTypePopup").css('display', 'block');
    } else {
      $("#truckTypePopup").css('display', 'none');
    }
  }
  showTrailerType() {
    if ($("#trailerTypePopup").css('display') == 'none' ) {
      $("#trailerTypePopup").css('display', 'block');
    } else {
      $("#trailerTypePopup").css('display', 'none');
    }
  }
  showDumpSite(idx: any) {
    if ($("#dumpSitePopup" + idx).css('display') == 'none' || !$("#dumpSitePopup" + idx).is(":visible")) {
      $("#dumpSitePopup" + idx).css('display', 'block');
    } else {
      $("#dumpSitePopup" + idx).css('display', 'none');
    }
  }
  showVendorName(idx: any) {
  
    if ($("#vendorNamePopup" + idx).css('display') == 'none' || !$("#vendorNamePopup" + idx).is(":visible")) {
      setTimeout(() => {
        
        $("#vendorNamePopup" + idx).css('display', 'block');
      }, 300);
    } else {
      $("#vendorNamePopup" + idx).css('display', 'none');
    }
  }
  showDriverName(idx: any) {
    if ($("#driverNamePopup" + idx).css('display') == 'none'  || !$("#driverNamePopup" + idx).is(":visible")) {
      setTimeout(() => {
        $("#driverNamePopup" + idx).css('display', 'block');
      }, 300);
    } else {
      $("#driverNamePopup" + idx).css('display', 'none');
    }
  }
  showVendorTruckName(idx: any) {
    if ($("#vendorTruckName" + idx).css('display') == 'none' || !$("#vendorTruckName" + idx).is(":visible")) {
      setTimeout(() => {
      $("#vendorTruckName" + idx).css('display', 'block');
    }, 300);
    } else {
      $("#vendorTruckName" + idx).css('display', 'none');
    }
  }
  showVendorTrailerName(idx: any) {
    if ($("#vendorTrailerPopup" + idx).css('display') == 'none' || !$("#vendorTrailerPopup" + idx).is(":visible")) {
      setTimeout(() => {
        $("#vendorTrailerPopup" + idx).css('display', 'block');
      }, 300);
    } else {
      $("#vendorTrailerPopup" + idx).css('display', 'none');
    }
  }
  ...
  get numberOfTrucks() {
    return this.ticketForm.get('number_of_trucks')?.value;
  }

  getRemainingTrucks(index: number) {
    let totalAssigned = 0;
    for (let i = 0; i < index; i++) {
      totalAssigned += Number(this.dispatches().at(i).get('number_of_trucks')?.value || 0);
    }
    return this.numberOfTrucks - totalAssigned;
  }

  get perHourRate() {
    return this.ticketForm.get('rate_per_hour_to_tc')?.value;
  }
  ...
  getProjectDetail() {
    ...
    this.tc_dispatch_service.getSingleData(data).subscribe(response => {
      ...
      if (response.status && response.data) {
        ...
        console.log("dump sites -->", this.dump_sites)
  ...
  getSelectedUserNames(): string {
    if (this.selected_approver.id === this.loggedinUser.id) {
      return this.loggedinUser.full_name;
    }
    const approver = this.approvers_list.find((item: any) => item.id === this.selected_approver.id || item.user?.id === this.selected_approver.id);
    return approver ? (approver.full_name ? approver.full_name : approver.user.full_name) : '';

  }

  saveSelectedApprovers(): void {
    // Hide the popup
    const popup = document.getElementById('jobApproversPopup');
    if (popup) {
  ...
  handleCheckboxChange(event: any): void {
    let checkedId = Number(event.target.getAttribute('data-value'));
    const approver = this.approvers_list.find((item: any) => item.id === checkedId);
    if (approver) {
      this.selected_approver = approver;
      this.ticketForm.get('approver_id')?.patchValue(approver?.id);
    } else {
      this.selected_approver = this.loggedinUser;
      this.ticketForm.get('approver_id')?.patchValue(this.loggedinUser?.id);
    }
    setTimeout(() => {
      this.showJobApprovers();
    }, 300)
  }
  ...
  setDumpSite(site: any, index: any) {
    this.rounds().at(index).get('dump_site')?.setValue(site.dump_site);
    setTimeout(() => {
      this.showDumpSite(index);
    }, 300);
  }
  ...
  setVendor(vendor: any, idx: any) {
    if (vendor !== '') {
      ...
      setTimeout(() => {
        this.showVendorName(idx);
      }, 300);

    } else {
      this.dispatches().at(idx).get('trucking_company_id')?.patchValue('Not on the list?');
      setTimeout(() => {
        this.showVendorName(idx);
      }, 300);
    }
  ...
  setTrailer(idx: any, trailer: any, ticket_id: any) {
    ...
    setTimeout(() => {
      this.showVendorTrailerName(idx);
    }, 300);
  }
