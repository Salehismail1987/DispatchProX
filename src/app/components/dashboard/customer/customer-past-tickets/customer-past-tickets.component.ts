...
@Component({
  selector: 'app-customer-past-tickets',
  templateUrl: './customer-past-tickets.component.html',
  styleUrls: ['./customer-past-tickets.component.css']
})
export class CustomerPastTicketsComponent implements OnInit {
  loggedinUser: any = {};
  message: any;
  ...
  ticketDetailView: boolean = false;
  ...
  getTicketListing() {
    ...
    this.ticket_service.getAllPastDispatches(formData).subscribe(response => {
  ...
  getStatusClass(status: string) {
    if (status == 'Approved' || status == 'Cancelled' || status == 'Declined') {
      status = 'Completed'
    } else if (status != 'Approved' && status != 'Cancelled' && status != 'Declined' && status != 'Pending') {
      status = 'Driving';
    }
    switch (status) {
      case 'Pending':
  ...
  getStatusClassForSingleTicket(status: string) {
    switch (status) {
      case 'Pending':
        return "mybtn status-btn bg-light-grey2 width-fit-content btn-text-very-small2";
        break;
  ...
  getApprovedTicketCount(ticket: any) {
    let isApprovedArr = ticket?.ticket_truck_type?.tickets?.filter((item: any) => item?.status == 'Approved');
    return isApprovedArr?.length;
  }

  getNumberOfDaysCount(ticket: any) {
    let datesArr: any = [];
    let isApprovedArr = ticket?.ticket_truck_type?.tickets?.map((item: any) => datesArr?.includes(item?.ticket_date) ? item : datesArr.push(item?.ticket_date));
    return datesArr?.length;
  }

  getTotalVolume(ticket: any) {
    let volume = 0;
    let rounds = 0;
    let totalVolume = 0;
    let isApprovedArr = ticket?.ticket_truck_type?.tickets?.map((item: any) => {
      rounds = 0;
      volume = 0;
      item?.ticket_truck_type_rounds?.map((round: any) => {
        if (round?.driver_start_time && round?.end_time) {
          rounds += 1;
        }
      });
      if (item?.driver_ticket && item?.driver_ticket?.truck) {
        volume += Number(item?.driver_ticket?.truck?.volume_capacity);
      }
      if (item?.driver_ticket && item?.driver_ticket?.trailer) {
        volume += Number(item?.driver_ticket?.trailer?.volume_capacity);
      }
      totalVolume += (rounds * volume);
    });
  ...
  setDetailTicket(ticket: any) {
    ...
    this.ticketDetailView = true;
    // this.opUpObj?.showPaperTicket();
  }

  closeDetailTicket() {
    this.ticketDetailView = false;
  }

  setDetailTicketOpenPopup(ticket: any) {
    this.singleTicketDetail = ticket;
    this.opUpObj?.showPaperTicket();
  }

  goToCreateDispatch(ticket: any) {
    this.router.navigate(['/customer-create-ticket'], { queryParams: { id: ticket?.ticket_truck_type_id } });
  }
